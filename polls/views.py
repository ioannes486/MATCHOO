...
def results(request):
    ...
    if request.method == 'POST':
        # 요청 받아서 문구 만들기
        res = request.POST
        user_message = '나는 {}고 {}고 {}고 {}고 {}고 {}해 나한테 부연설명 없이 단답형으로 한국여행지 한 곳을 여행지 이름만 추천해줘'.format(res.get('trip'), res.get('type'), res.get('movement'), res.get('pop'), res.get('with'), res.get('how'), )
